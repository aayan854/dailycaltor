...
export function SmartMealBuilder() {
  const [inputs, setInputs] = useState<MealBuilderInputs>({
    targetCalories: 2000,
    targetProtein: 150,
    mealsPerDay: 3,
    foodsToAvoid: [],
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-2 gap-4 text-center">
            <div className="p-4 bg-background rounded-lg shadow-sm border">
              ...
              <div className="text-2xl font-bold">{plan.totalDailyCalories.toFixed(0)}</div>
            </div>
            <div className="p-4 bg-background rounded-lg shadow-sm border">
              <div className="text-sm text-muted-foreground mb-1">Total Protein (g)</div>
              <div className="text-2xl font-bold text-primary">{plan.totalDailyProtein.toFixed(0)}</div>
            </div>
          </div>
